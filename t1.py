...
def exec_query(my_console, query):
    my_console.stdout = io.StringIO()
    real_stdout = sys.stdout
    sys.stdout = my_console.stdout
    my_console.onecmd(query)
    sys.stdout = real_stdout
    return my_console.stdout.getvalue().strip()

# Verify that the state was created
created_state = exec_query(my_console, 'show State {}'.format(state_id_0))
if "Arizona" not in created_state:
    print("\nFAIL: State not found in database")
else:
    print("\nState found in database")

# Similarly, verify other created instances
# ...
